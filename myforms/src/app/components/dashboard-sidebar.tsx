    ...
    return (
        <div className="h-full w-[300px] dark:border-r-gray-600 border-r-gray-200 border-r-[1px] flex flex-col text-xl text-light-text dark:text-dark-text font-bold">
            <Link
                ...
                className={(pathname === '/dashboard/forms' ? `bg-light-sec dark:bg-dark-sec` : ``) + ` w-[292px] mx-1 mt-0.5 h-12 px-4 py-8 hover:bg-light-sec dark:hover:bg-dark-sec 
                flex items-center justify-center dark:border-t-gray-600 border-t-gray-200 border-t-[1px] rounded-lg
                transition-all hover:shadow-md hover:m-0 hover:h-16 hover:z-10`}
            >
                <FontAwesomeIcon icon={faNewspaper} />
                <span className="ml-2">My forms</span>
            </Link>
            <Link href={'/dashboard/trashcan'}
                className={(pathname === '/dashboard/trashcan' ? `bg-light-sec dark:bg-dark-sec` : ``) + ` w-[292px] mx-1 mt-0.5 h-12 px-4 py-8 hover:bg-light-sec dark:hover:bg-dark-sec 
                flex items-center justify-center dark:border-t-gray-600 border-t-gray-200 border-t-[1px] rounded-lg
                transition-all hover:shadow-md hover:m-0 hover:h-16 hover:z-10`}
            >
